...
appTouch.prototype = {
	move : function(e) {
		...
		sheight = monitor.offsetHeight / target.offsetHeight
				* monitor.offsetHeight, // 自定义滚动条的长度
		st = (target.offsetHeight - monitor.offsetHeight)
				/ (monitor.offsetHeight - sheight), // 移动块对应滚轮单位长度
		tslow = 4, // 到顶/底减基数
		tMove = 0, // 滑块到顶top值
		...
		function moveIn(e) {
			if (flg) {
				...
				if (move > tMove) {
					(move - tMove) / tslow + tMove > tMoveL ? move = tMoveL
							: move = (move - tMove) / tslow + tMove

				} else if (move < bMove)
					(move - bMove) / tslow + bMove < bMoveL ? move = bMoveL
							: move = (move - bMove) / tslow + bMove;
